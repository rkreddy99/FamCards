...
class DatabaseService {
  ...
  Future updatePersonName(String name, String uid) async {
    return await personCollection.document(uid).updateData({'name': name});
  }

  Future addDownloadUrl(
      DateTime time, StorageReference _storage, String text) async {
  ...
  Future deleteDownloadUrl(String fieldName) async {
    StorageReference _storage =
        FirebaseStorage(storageBucket: 'gs://docs-ac473.appspot.com')
            .ref()
            .child('$uid/$fieldName.png');
  ...
  List<PersonDetails> _printPersonsData(QuerySnapshot snapshot) {
    return snapshot.documents.map((e) {
      ...
      return PersonDetails(
          name: e.data['name'], uid: e.documentID, data: e.data);
    }).toList();
  }
